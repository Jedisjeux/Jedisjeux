...
class LoadTopicsOfGamePlayCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        foreach ($this->getTopics() as $data) {
            $output->writeln(sprintf("Loading <info>%s</info> comment of <info>%s</info> gameplay", $data['id'], $data['gameplay_id']));
            if ($gamePlayId !== $data['gameplay_id']) {
                $topic = $this->getTopicFactory()->createForGamePlay($data['gameplay_id']);
            }

            $gamePlayId = $data['gameplay_id'];
        }
